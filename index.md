...
数据转换（三）	20
数据转换（四）	21
数据转换（五）——reshape2包的使用（详细可参考R语言实战）	22
数据转换（六）——tidyr包的使用	23
数据处理——dplyr包的使用	25
函数介绍——R语言中的函数	28
函数介绍——函数的选项参数	29
函数介绍——数学统计函数	30
函数介绍——描述性统计函数（R语言实战p.131）	31
函数介绍——频数统计函数	33
函数介绍——独立性检验函数	36
函数介绍——相关性分析函数	38
函数介绍——相关性检验函数	39
...
abs()绝对值，sqrt（）平方根，log（x，base=默认为e），log10（），exp（x）计算e的x次幂, prod()乘积，quantile(x,c(0.4,0.5,0.8)分位数
...
分别是矩阵的内积（对应元素相乘）以及矩阵的外积（通俗意义的矩阵乘法，m*n, n*p, m*p）
...
在R中，名义型变量和有序型变量被称为因子，factor，这些分类变量的可能只被称为一个水平，level，例如good、better、best，都被称为一个level，由这些水平值构成的向量就称为因子。在很多绘图函数中，输入的数据也必须是因子类型。
...
使用load()函数可以直接打开Rdata文件。文件中的对象会覆盖之前的对象。
用save（）函数保存
数据转换（一）
...
as.data.frame()函数还可以将数据转换为数据框格式
unlist()可以用于将列表转化成向量
...
rowsums()和colmeans()函数可以计算行的和还有列的平均数
...
比如，可以使用rbind函数合并iris数据集的1-10行数据与1-15行数据，再使用distinct函数去除多余行：
dplyr::distinct(rbind(iris[1:10,],iris[1:15,]))
以上代码的运行结果只会保留，1-15行的数据，因为1-10行数据是重复的
...
如果加上前面加上负号则是进行反向排序（从大到小），desc（）也可以。
dplyr::arrange(iris,-Sepal.Length)
关于dplyr的统计函数：
...
R中一个非常有用的符号：%>% 这个符号是链式操作符，它的功能是用于实现将一个函数的输出传递给下一个函数，作为下一个函数的输入，在键盘上可以用Ctrl+shift+M的快捷键打出来，有点类似于“且”的概念。
